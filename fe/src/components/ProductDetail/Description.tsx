...
const Description : React.FC = ( props : Props) => {
    ...
    useEffect(() => {
        setImageShow(props.image ? JSON.parse(props.image)[0] : "")
    }, [props]);
                ...
                <div className={'grid gap-4'}>
                    {
                        isLoading ?
                            ...
                            :
                            <img src={imageShow} alt="ảnh" className={'border cursor-pointer w-full h-full p-4 object-cover'}/>
                    }
                    <div className={'grid grid-cols-4 gap-4'}>
                        {
                            isLoading ?
                                ...
                                <>
                                    {
                                        image && JSON.parse(image).map((img: string, index: number) => (
                                            <div key={index}>
                                                <img onClick={ ()=> setImageShow(img)} src={img} alt={props.title} className={'w-full h-full border-2 cursor-pointer p-1'}/>
                                            </div>
                                        ))
                                    }

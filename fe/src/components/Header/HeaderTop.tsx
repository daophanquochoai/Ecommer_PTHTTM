...
import {setLogOut, settingAboutWebsite} from "../../Utils/Helper.tsx";
...
const HeaderTop : React.FC = () => {
    // state
    const {isLogin, setIsLogin, info, contactInformation} = useContext(AppContext)
    ...
    return (
        <div className={'bg-white justify-end flex md:justify-between items-center px-[5%] pt-3 pb-1 border-b-2'}>
            <div className={'hidden md:flex'}>
                <p className={'text-gray-500 text-sm'}>Call Us : {contactInformation.supportPhoneNumber}</p>
                <p className={'ml-3 text-gray-500 text-sm'}>Email : { contactInformation.contactEmail}</p>
            </div>

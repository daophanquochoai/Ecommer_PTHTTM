...
const AcceptCart = () => {
    ...
    const columns: TableProps<DataType>['columns'] = [
        {
            ...
            render : (item) => (
                <div className={'flex items-center gap-2 ps-[35%]'}>
                    <img className={'w-[50px] h-[50px]'} src={JSON.parse(item.image) ? JSON.parse(item.image)[0] : ""}/>
    ...
    useEffect(() => {
       ...
       info.addresses.forEach(item => {
            ...
            {
                setAddressDefault(item.address_name)
                setInputValue(item.address_name)
            }
            arr.push({
    ...
    const onInputChange = (e) => {
        setInputValue(e.target.value); // cập nhật input address
    };
    ...
    return (
        <div>
            <div className={'flex items-center gap-2 text-xl font-bold text-red-500 mx-12 mt-4 cursor-pointer'} onClick={handleClickBack}>
                <IoMdArrowRoundBack />
                <p>BACK</p>
            </div>

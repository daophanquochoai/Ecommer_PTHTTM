    ...
    return (
        <div className={`${!openCart  ? 'right-[-400px]' : 'right-0'} transition-all duration-300 fixed bg-white z-20 h-[100vh] w-[400px] top-0 p-4`}>
            <div className={'flex justify-end text-3xl cursor-pointer'}>
                <IoClose onClick={() => setOpenCart(false)}/>
            </div>
            <div>
                <div>
                ...
                <button className={'mt-6 w-full bg-red-500 p-2 text-xl text-white'} onClick={ () => { navigate('/cart'); setOpenCart(false)}}>Go to Cart</button>
            </div>
        </div>
    );
};
